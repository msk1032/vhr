  ...
  methods: {
    doAddRole() {
      if (this.role.name && this.role.nameZh) {
        ...
        this.$message.error({ message: "data not null" });
      }
    },
    initRoles() {
    ...
    doUpdate(id, index) {
      ...
      this.putRequest("/system/basic/permiss/update/" + id, selectedKeys).then(
        (res) => {
          if (res) {
            this.activeName = "";
          }
    ...
    deleteRole(role) {
      this.$confirm(
        ...
        .then(() => {
          this.deleteRequest("/system/basic/permiss/" + role.id).then(
            (resp) => {
              if (resp) {
                this.initRoles();
              }
